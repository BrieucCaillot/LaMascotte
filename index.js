...
// app post newsletter
	...
	app.post('/booking', function(req, res){
	    ...
	    if (//req.body.lastname.match(regExpText) && 
	    	...
	    	// req.body.tel.match(regExpNumber) && 
	    	req.body.tel.length >=5 ){
	        co.query(q, function (error, results, fields){
	            if (error) return console.log(error);
	            res.status(201).send('Votre réservation à bien été enregistrée pour ' + body.persons.value + ' personnes au nom de ' + body.lastname.value + 'le' + body.date_booking.value + 'à' + body.time_booking);
	        	console.log(results);
	        });
	    }

	    // send error lastname 
	    else if (req.body.lastname.length =0 /*&& req.body.lastname.match(regExpText)*/){
			res.status(400).send('Veuillez remplir correctement le champ "Nom" pour enregistrer votre réservation');
	    }
	    // send error date_booking
	    else if (req.body.date_booking.length =0 /*&& req.body.date_booking.match(regExpDate)*/){
